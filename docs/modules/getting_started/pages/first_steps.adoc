...
After you went through the xref:installation.adoc[] of the Operator and its dependencies, you will now go through the steps to set up a Superset instance and connect to the instance afterwards.
...
A secret with the necessary credentials must be created, this entails database connection credentials as well as an admin account for Superset itself. Create a file called `superset-credentials.yaml`:
...
[source,bash]
include::example$code/getting_started.sh[tag=apply-superset-credentials]
...
[source,bash]
include::example$code/getting_started.sh[tag=apply-superset-cluster]
...
[source,bash]
include::example$code/getting_started.sh[tag=wait-supersetdb]
...
Look at the xref:ROOT:usage.adoc[Usage page] to find out more about configuring your Superset instance or have a look at the Superset documentation to link:https://superset.apache.org/docs/creating-charts-dashboards/creating-your-first-dashboard[create your first dashboard].
