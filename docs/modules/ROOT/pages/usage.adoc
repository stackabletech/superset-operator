...
The AuthenticationClass is then referenced in the SupersetCluster resource like so:
...
<2> The default role that all users are assigned to upon logging in

Users that log in with LDAP are assigned a default https://superset.apache.org/docs/security/#roles[Role] upon logging in which is specified with the `userRegistrationRole` property.
...
=== LDAP
Superset supports assigning https://superset.apache.org/docs/security/#roles[Roles] to users based on their LDAP group membership, but the Stackable Operator does not support this yet.
All the users logging in via LDAP get assigned the same role which you can configure via the attribute `authenticationConfig.userRegistrationRole` on the `SupersetCluster` object:
