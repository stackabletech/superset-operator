...
In order to map roles from OPA into Superset, we expect rego rules with a rule named `user_roles`.
In the below example two users `admin` and `testuser` have roles defined as rego rule.
...
You can instruct Superset to use the rego rule as follows:
