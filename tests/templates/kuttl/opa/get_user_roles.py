...
login_page = session.get(
    f"http://superset-external.{namespace}.svc.cluster.local:8088/login/"
)
...
session.get(
    f"http://superset-external.{namespace}.svc.cluster.local:8088/api/v1/dashboard/"
)

# Get user roles
user_roles = set(
    session.get(
        f"http://superset-external.{namespace}.svc.cluster.local:8088/api/v1/me/roles/"
    )
    .json()["result"]["roles"]
    .keys()
)
