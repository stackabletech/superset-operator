...
VERSION := $(shell cargo metadata --format-version 1 | jq -r '.packages[] | select(.name=="stackable-superset-operator") | .version')
...
docker-build:
	docker build --force-rm --build-arg VERSION=${VERSION} -t "docker.stackable.tech/stackable/superset-operator:${VERSION}" -f docker/Dockerfile .

docker-build-latest: docker-build
	docker tag "docker.stackable.tech/stackable/superset-operator:${VERSION}" \
...
chart-clean:
	...
	rm -rf deploy/helm/superset-operator/crds

version:
	yq eval -i '.version = strenv(VERSION) | .appVersion = strenv(VERSION)' /dev/stdin < deploy/helm/superset-operator/Chart.yaml
	yq eval -i '.version = strenv(DOCS_VERSION) | .prerelease = strenv(IS_NIGHTLY) != ""' /dev/stdin < docs/antora.yml

config:
	if [ -d "deploy/config-spec/" ]; then\
		mkdir -p deploy/helm/superset-operator/configs;\
		cp -r deploy/config-spec/* deploy/helm/superset-operator/configs;\
	fi

crds:
	mkdir -p deploy/helm/superset-operator/crds
	cargo run --bin stackable-superset-operator -- crd | yq eval '.metadata.annotations["helm.sh/resource-policy"]="keep"' - > deploy/helm/superset-operator/crds/crds.yaml
